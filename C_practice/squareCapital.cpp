...
// In this exercise you are asked to implement the accumulator class.
// This class can be used to accumulate values over time, similarly to
// what std::accumulate (https://en.cppreference.com/w/cpp/algorithm/accumulate)
// does over a range of iterators.
// The difference being that you push values to the accumulator class one by one.
//
// The class declaration is provided to you and you cannot modify it.
// It uses PIMPL (https://en.cppreference.com/w/cpp/language/pimpl) to hide the
// private data so that you can customize it without affecting the declaration.
//
// This exercise implements a bunch of check functions that will validate
// various aspects of your implementation. The goal of this exercise is to
// implement the accumulator class so that all checks pass.
//
// Some check functions will not compile if your implementation is not appropriate.
// To let you implement the class gradually, you can disable checks at compile time
// to avoid these compilation errors and carry on with you work on the first checks.
// That can be done through the check_xxx_enabled booleans.
// It is recommended that you enable checks one by one so that you can work through
// the problem gradually.
//
// For convenience, you can write any code you like in the function "user_defined_code".
// This can be helpful if you want to debug your code by printing to the standard
// output. Don't forget to remove the output when you're done, as HackRank doesn't
// expect any user-defined output in the test cases.
//
// It is up to you to figure out how to implement the features required by the various
// checks. You can read the check function's implementation and comments to better
...
constexpr bool check_001_enabled = false;
